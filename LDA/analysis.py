...
''' Set here if your work based on LDA or LSI '''

lda = models.LdaModel.load('../Save/modelLDA.lda')
#lda = models.LsiModel.load('../Save/modelLDA.lsi')
...
if similarities_between_pairs == True:
    ...
    index = similarities.MatrixSimilarity(lda[corpus_tfidf]) # only possible if the total memory required is lower than the RAM. In any other case, you should use similarities. Can also add: ,num_features=len(dictionary)
...
#pprint(lda.get_document_topics(corpus_tfidf[680]))
